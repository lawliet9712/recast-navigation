		...
		for (int dir = 0; dir < 4; ++dir)
		{
			...
			if (rcGetCon(cs, dir) != RC_NOT_CONNECTED)
			{
				...
				if (nr & RC_BORDER_REG) // Do not take borders into account.
					continue;
                // 这里表示邻居已经合并到 region 了，并且不是当前这个 region，先跳过这个节点，后面 expandRegion 会设置该节点
				if (nr != 0 && nr != r)
				{
					ar = nr;
					break;
					      ...
					      bool fillStack)
{
	...
	else // use cells in the input stack
	{
		...
		for (int j=0; j<stack.size(); j++)
		{
			int i = stack[j].index;
			if (srcReg[i] != 0)
				stack[j].index = -1; // 已经填充过了，则打个 tag
		}
	}
	...
	while (stack.size() > 0)
	{
		...
		for (int j = 0; j < stack.size(); j++)
		{
			...
			for (int dir = 0; dir < 4; ++dir)
			{
                ...
                // 邻居有相同的 area id，并且不是 height filed 的边界
				if (srcReg[ai] > 0 && (srcReg[ai] & RC_BORDER_REG) == 0)
				{
					if ((int)srcDist[ai]+2 < (int)d2)
					{
						r = srcReg[ai];
						d2 = srcDist[ai]+2; // 固定距离为 2
					}
				}
			}
			if (r)
							  ...
							  unsigned short loglevelsPerStack) // the levels per stack (2 in our case) as a bit shift
{
    ...
    // loglevelsPerStack 默认为 1，因此这里其实是 startLevel / 2
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				int sId = startLevel - level; // 距离越远的 span，stack 越低
				if (sId >= (int)nbStacks) // 8 级一个批次，这里属于下一个批次的，比如现在是 16-8 的批次，那么这里属于下一个批次 8-0
					continue;
				if (sId < 0)
					sId = 0;
...
static bool canMergeWithRegion(const rcRegion& rega, const rcRegion& regb)
{
	...
	if (n > 1) // 这样表示这个 region 已经合并过一次了，不能再合并，否则合并好的区域会分裂
		return false;
					...
					const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	unsigned short level = (chf.maxDistance+1) & ~1; // 先前构建距离场的时候，dist 数组里面最大的距离值，& ~1 保证为偶数
...
//	const int expandIters = 4 + walkableRadius * 2;
    ...
    // 有 border size 的情况下，先对 border size 和中间的 Tile 之间的间隔进行 build region
    //   :''''''''': h
    //   : +-----+ :
    //   : |     | :
    //   : |     | <------ 先对包裹住的这 4 条边框进行区域划分。
	...
	while (level > 0)
	{
		...
		sId = (sId+1) & (NB_STACKS-1); // sid 范围 [0,7]，NB_STACK 为 8

//		ctx->startTimer(RC_TIMER_DIVIDE_TO_LEVELS);
// 这里的流程是：
/*
 * 由内向外进行注水，先处理最远的，也就是最中间的 Span
 * 1. 将不同距离的 dist，按照 2~3 的间隔，比如 16,15,14 或者 15,14，dist 值为这种情况的放在同一个 stack
     * 为什么会有 3
     * // 假设 startLevel 为 7（startLevel = (maxDist - 2) / 2），那么这里理论上只有 2 种距离的 span，比如 dist 为 14 和 15 的其实是同一个 Level
     * // 但是还有一种，如果最大距离 maxDist 为偶数（比如 16），那么最远距离的同一个 level 可能存在 3 种距离，16,15,14
 * 2. sortCellsByLevel 这里主要把 Span 放到对应 Stack，但是这里处理有限，Level > 16，那么表示这块区域比较大，需要多次处理（每次 16 个距离），进行 sortCell，然后 floodRegion，expandRegion
 */
		if (sId == 0)
            // level <= 16, 只会触发一次 sortCell，所以这里处理的次数为 level / 16(向上取整)
            // 每次处理 8 个 Stack，处理完之后如果 level 还 >= 0, 继续 sortCell
			sortCellsByLevel(level, chf, srcReg, NB_STACKS, lvlStacks, 1);
		else
            // 合并 stack（2个level合为一个批次，如 01，23）
...
//		ctx->stopTimer(RC_TIMER_DIVIDE_TO_LEVELS);

		{
			...
			// Expand current regions until no empty connected cells found. 第一次 sortCells 之后的 expandRegions 理论上什么都做不了，因为这个时候除了 border 区域，其他都是空的 region id
            // floodRegion 之后，可能出现，相邻的 span，region id 不一样，area id 一样，这个时候就将其合并为相同的 region
